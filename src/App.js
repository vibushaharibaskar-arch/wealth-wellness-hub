...
const SHOCK_SCENARIOS = [
  ...
  { key: "market",     label: "Market Crash", icon: "📉", desc: "25% investment drop"      },
  { key: "medical",    label: "Medical",      icon: "🏥", desc: "Sudden $6k expense"       },
...
// ─── FINLEY SVG — simple arctic fox ──────────────────────────────────────────
function Finley({ mood = "happy", size = 120 }) {
  return (
    <div style={{ width: size, height: size, flexShrink: 0 }}>
      <svg width="100%" height="100%" viewBox="0 0 100 100" fill="none">
        <ellipse cx="50" cy="75" rx="18" ry="14" fill="white" stroke="#cbd5e1" strokeWidth="1.5"/>
        <ellipse cx="74" cy="78" rx="10" ry="7" fill="white" stroke="#cbd5e1" strokeWidth="1.5"/>
        <ellipse cx="81" cy="78" rx="4" ry="4" fill="#e0f2fe"/>
        <polygon points="34,42 28,22 42,36" fill="white" stroke="#cbd5e1" strokeWidth="1.5" strokeLinejoin="round"/>
        <polygon points="35,40 30,25 40,36" fill="#fecdd3"/>
        <polygon points="66,42 72,22 58,36" fill="white" stroke="#cbd5e1" strokeWidth="1.5" strokeLinejoin="round"/>
        <polygon points="65,40 70,25 60,36" fill="#fecdd3"/>
        <ellipse cx="50" cy="54" rx="22" ry="20" fill="white" stroke="#cbd5e1" strokeWidth="1.5"/>
        <ellipse cx="50" cy="62" rx="10" ry="7" fill="#f1f5f9"/>
        <circle cx="42" cy="51" r="4" fill="#1e293b"/>
        <circle cx="58" cy="51" r="4" fill="#1e293b"/>
        <circle cx="43.5" cy="49.5" r="1.5" fill="white"/>
        <circle cx="59.5" cy="49.5" r="1.5" fill="white"/>
        <ellipse cx="50" cy="60" rx="3" ry="2" fill="#475569"/>
        {mood === "happy"   && <path d="M45,64 Q50,68 55,64" stroke="#475569" strokeWidth="1.5" strokeLinecap="round" fill="none"/>}
        {mood === "neutral" && <path d="M45,65 Q50,65 55,65" stroke="#475569" strokeWidth="1.5" strokeLinecap="round" fill="none"/>}
        {mood === "worried" && <path d="M45,66 Q50,63 55,66" stroke="#475569" strokeWidth="1.5" strokeLinecap="round" fill="none"/>}
      </svg>
    </div>
  );
}
...
function Card({ children, title, icon, style = {} }) {
  return (
    <div style={{ background: "white", borderRadius: 28, border: "1px solid #e0f2fe", boxShadow: "0 4px 24px rgba(14,165,233,0.07)", padding: "36px 40px", display: "flex", flexDirection: "column", ...style }}>
      {title && (
...
// ─── CHAT WIDGET ─────────────────────────────────────────────────────────────
function ChatWidget({ mood, profile }) {
  ...
  return (
    <div style={{ position: "fixed", bottom: 36, right: 36, zIndex: 9999 }}>
      {!open && (
        <div style={{ position: "absolute", bottom: 104, right: 0, background: "white", border: "1px solid #bae6fd", borderRadius: 20, padding: "12px 18px", fontSize: 13, color: "#0c4a6e", fontWeight: 600, maxWidth: 240, boxShadow: "0 8px 24px rgba(14,165,233,0.15)", lineHeight: 1.5 }}>
          {msgs[msgIdx]}
          <div style={{ position: "absolute", bottom: -8, right: 30, width: 0, height: 0, borderLeft: "8px solid transparent", borderRight: "8px solid transparent", borderTop: "8px solid #bae6fd" }} />
        </div>
      )}

      {open && (
        <div style={{ position: "absolute", bottom: 104, right: 0, width: 420, background: "white", borderRadius: 32, boxShadow: "0 32px 80px rgba(12,74,110,0.18)", border: "1px solid #e0f2fe", display: "flex", flexDirection: "column", height: 580, overflow: "hidden" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "24px 28px", display: "flex", flexDirection: "column", gap: 16, background: "#f8faff" }}>
            {messages.map((m, i) => (
              <div key={i} style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
                <div style={{ maxWidth: "82%", padding: "14px 18px", borderRadius: 20, fontSize: 14, lineHeight: 1.65, ...(m.role === "user" ? { background: "#0c4a6e", color: "white", borderTopRightRadius: 4 } : { background: "white", color: "#0c4a6e", borderTopLeftRadius: 4, border: "1px solid #e0f2fe" }) }}>
                  {m.text}
                </div>
              </div>
            ))}
            {busy && <div style={{ display: "flex", justifyContent: "flex-start" }}><div style={{ background: "white", border: "1px solid #e0f2fe", borderTopLeftRadius: 4, borderRadius: 20, padding: "14px 18px", fontSize: 14, color: "#94a3b8" }}>Finley is thinking ❄️</div></div>}
            <div ref={bottomRef} />
          </div>
          ...
          <div style={{ padding: "16px 20px 20px", background: "white", borderTop: "1px solid #f0f9ff", display: "flex", gap: 10, flexShrink: 0 }}>
            ...
            <button onClick={send} disabled={busy}
              style={{ width: 50, height: 50, background: "#0c4a6e", color: "white", border: "none", borderRadius: 16, cursor: "pointer", fontSize: 18, flexShrink: 0 }}>→</button>
          </div>
        </div>
      )}
...
export default function App() {
  ...
  async function fetchScore() {
    try {
      const d = await post("/score", { assets: profile.assets, liabilities: profile.liabilities, monthly: profile.monthly });
      setScoreData(d);
    } catch {}
  }
  ...
  async function fetchBehaviour() {
    try {
      const d = await post("/behaviour", { monthly: profile.monthly, reminders: profile.reminders, goals: profile.goals });
      setBehaviourData(d);
    } catch {}
  }
  ...
  async function fetchShock() {
    ...
    try {
      const d = await post("/shock", { scenario: shockScenario, savings: profile.assets.savings, investments: profile.assets.investments, income: profile.monthly.income, expenses: profile.monthly.expenses, debt: profile.liabilities.debt });
      setShockData(d);
    } catch {}
  ...
  async function fetchMacro() {
    ...
    try {
      const d = await post("/macro", { scenario: macroScenario, investments: profile.assets.investments, expenses: profile.monthly.expenses, debt: profile.liabilities.debt, income: profile.monthly.income });
      setMacroData(d);
    } catch {}
  ...
  const btnPrimary = { background: "linear-gradient(135deg,#0c4a6e,#0ea5e9)", color: "white", padding: "14px 28px", borderRadius: 14, fontSize: 15, fontWeight: 700, border: "none", cursor: "pointer", boxShadow: "0 8px 20px rgba(14,165,233,0.3)", display: "inline-flex", alignItems: "center", gap: 8 };
  const btnGhost   = { background: "#f0f9ff", color: "#0c4a6e", padding: "14px 28px", borderRadius: 14, fontSize: 15, fontWeight: 700, border: "1px solid #e0f2fe", cursor: "pointer" };
  ...
  return (
    <>
        ...
        <aside style={{ width: 280, background: "white", borderRight: "1px solid #e0f2fe", display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100vh", flexShrink: 0 }}>
          <div style={{ padding: "36px 32px 28px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 10 }}>
              ...
              <h1 style={{ fontSize: 22, fontWeight: 800, color: "#0c4a6e", letterSpacing: -0.5 }}>Wealth<span style={{ color: "#0ea5e9" }}>Wellness</span></h1>
            </div>
          ...
          <nav style={{ flex: 1, padding: "0 16px", display: "flex", flexDirection: "column", gap: 4, overflowY: "auto" }}>
            {TABS.map(t => (
              <button key={t.id} onClick={() => setActiveTab(t.id)} style={{ width: "100%", display: "flex", alignItems: "center", gap: 14, padding: "15px 18px", borderRadius: 18, fontSize: 14, fontWeight: 700, border: "none", cursor: "pointer", textAlign: "left", transition: "all 0.15s", background: activeTab === t.id ? "#0ea5e9" : "transparent", color: activeTab === t.id ? "white" : "#94a3b8", boxShadow: activeTab === t.id ? "0 8px 20px rgba(14,165,233,0.25)" : "none" }}>
                <span style={{ fontSize: 18 }}>{t.icon}</span>{t.id}
              </button>
            ))}
          </nav>
          ...
          <div style={{ padding: "20px 20px 28px" }}>
            <div style={{ background: "#f0f9ff", border: "1px solid #e0f2fe", borderRadius: 22, padding: "18px 20px", display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{ width: 56, height: 56, borderRadius: 16, background: "white", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
        ...
        <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          <header style={{ background: "rgba(255,255,255,0.9)", backdropFilter: "blur(12px)", borderBottom: "1px solid #e0f2fe", padding: "0 44px", height: 88, display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 40, flexShrink: 0 }}>
            <div>
              ...
              <p style={{ fontSize: 13, color: "#94a3b8", marginTop: 3 }}>Welcome back, {profile.name}! Here's your arctic financial update.</p>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 18px", background: "#f0f9ff", borderRadius: 14, border: "1px solid #e0f2fe" }}>
                <span>📅</span>
                <span style={{ fontSize: 13, fontWeight: 700, color: "#0c4a6e" }}>March 10, 2026</span>
              </div>
                ...
                {editingName ? (
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <input autoFocus value={nameDraft}
                      ...
                      onKeyDown={e => {
                        if (e.key === "Enter")  { setProfile(p => ({ ...p, name: nameDraft })); setEditingName(false); }
                        if (e.key === "Escape") { setNameDraft(profile.name); setEditingName(false); }
                      }}
                      style={{ fontSize: 14, fontWeight: 800, color: "#0c4a6e", border: "2px solid #0ea5e9", borderRadius: 10, padding: "6px 12px", outline: "none", fontFamily: "inherit", width: 140 }} />
                    <button onClick={() => { setProfile(p => ({ ...p, name: nameDraft })); setEditingName(false); }} style={{ background: "#0ea5e9", color: "white", border: "none", borderRadius: 8, padding: "6px 12px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>Save</button>
                    <button onClick={() => { setNameDraft(profile.name); setEditingName(false); }} style={{ background: "#f0f9ff", color: "#94a3b8", border: "1px solid #e0f2fe", borderRadius: 8, padding: "6px 10px", fontSize: 13, cursor: "pointer" }}>✕</button>
                  </div>
                ) : (
                  <div style={{ textAlign: "right", cursor: "pointer" }} onClick={() => { setNameDraft(profile.name); setEditingName(true); }} title="Click to edit your name">
                    <p style={{ fontSize: 14, fontWeight: 800, color: "#0c4a6e" }}>{profile.name}</p>
                    <p style={{ fontSize: 10, color: "#94a3b8", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.12em", marginTop: 2 }}>Premium Member ✏️</p>
                  </div>
                )}
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: "44px" }}>
            ...
            {activeTab === "Overview" && (
              <div style={{ display: "flex", flexDirection: "column", gap: 36 }}>
                  ...
                  <div style={{ background: "linear-gradient(135deg,#0c4a6e 0%,#083344 100%)", borderRadius: 28, padding: "44px 48px", display: "flex", alignItems: "center", justifyContent: "space-between", overflow: "hidden", position: "relative", boxShadow: "0 20px 60px rgba(12,74,110,0.22)" }}>
                    <div style={{ position: "relative", zIndex: 1, maxWidth: 460 }}>
                      ...
                      <h3 style={{ fontSize: 36, fontWeight: 800, color: "white", letterSpacing: -0.5, marginBottom: 18, lineHeight: 1.2 }}>
                        {profile.name.split(" ")[0]}, your finances are{" "}
                ...
                {/* Goals */}
                <div>
                  <h3 style={{ fontSize: 20, fontWeight: 800, color: "#0c4a6e", marginBottom: 24 }}>🎯 Life Goals</h3>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 24 }}>
                    {profile.goals.map((g, i) => {
                      const pct = Math.min(100, Math.round((g.current / g.target) * 100));
                      const gc  = pct >= 75 ? "#10b981" : pct >= 40 ? "#0ea5e9" : "#f59e0b";
                      return (
                        <Card key={i} title={g.name} icon="🎯">
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 16 }}>
                            <p style={{ fontSize: 28, fontWeight: 800, color: "#0c4a6e", fontFamily: "monospace" }}>${g.current.toLocaleString()}</p>
                            <p style={{ fontSize: 12, color: "#94a3b8" }}>of ${g.target.toLocaleString()}</p>
                          </div>
                          <ProgressBar value={g.current} max={g.target} color={gc} />
                          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 12 }}>
                            <span style={{ fontSize: 13, fontWeight: 700, color: gc }}>{pct}% complete</span>
                            <span style={{ fontSize: 12, color: "#94a3b8" }}>Due {g.deadline}</span>
                          </div>
                        </Card>
                      );
                    })}
                ...
                {/* Reminders */}
                <div>
                  <h3 style={{ fontSize: 20, fontWeight: 800, color: "#0c4a6e", marginBottom: 24 }}>📅 Upcoming Reminders</h3>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 24 }}>
                    {profile.reminders.map((r, i) => {
                      const s = r.type === "Essential" ? { tag: "#dcfce7", tagText: "#166534", border: "#bbf7d0" } : r.type === "Cuttable" ? { tag: "#ffe4e6", tagText: "#9f1239", border: "#fecdd3" } : { tag: "#fef9c3", tagText: "#854d0e", border: "#fde68a" };
                      return (
                        <div key={i} style={{ background: "white", borderRadius: 24, border: "1px solid #e0f2fe", padding: "28px 30px", transition: "all 0.2s" }}
              ...
              <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", flexDirection: "column", gap: 44 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 28 }}>
                  {[
                    ...
                    { label: "Debt Ratio",       val: `${scoreData?.debtRatio ?? 0}%`,    target: "<40%", color: "#f43f5e", pct: Math.max(0, 100 - (scoreData?.debtRatio ?? 0)) },
                  ].map(s => (
            ...
            {activeTab === "Life Shock" && (
              <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", flexDirection: "column", gap: 40 }}>
                <div style={{ textAlign: "center" }}>
                  ...
                  <p style={{ color: "#64748b", marginTop: 12, fontSize: 15, lineHeight: 1.7 }}>Simulate unexpected events to see how resilient your finances are. ❄️</p>
                </div>

                <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 18 }}>
                  {SHOCK_SCENARIOS.map(sc => (
                    <button key={sc.key} onClick={() => setShockScenario(sc.key)} style={{ padding: "22px 20px", borderRadius: 20, background: "white", border: `2px solid ${shockScenario === sc.key ? "#0ea5e9" : "#e0f2fe"}`, cursor: "pointer", textAlign: "left", transition: "all 0.15s", boxShadow: shockScenario === sc.key ? "0 0 0 4px rgba(14,165,233,0.12)" : "none" }}>
            ...
            {activeTab === "Behaviour" && (
              <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", flexDirection: "column", gap: 36 }}>
                ...
                ) : <p style={{ color: "#94a3b8", textAlign: "center", padding: 48 }}>Loading behaviour data…</p>}
              </div>
            )}
              ...
              <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", flexDirection: "column", gap: 36 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                  {MACRO_SCENARIOS.map(sc => (
                    <button key={sc.key} onClick={() => setMacroScenario(sc.key)} style={{ padding: "26px 28px", borderRadius: 20, background: "white", border: `2px solid ${macroScenario === sc.key ? "#0ea5e9" : "#e0f2fe"}`, cursor: "pointer", textAlign: "left", transition: "all 0.15s", boxShadow: macroScenario === sc.key ? "0 0 0 4px rgba(14,165,233,0.12)" : "none" }}>
            ...
            {activeTab === "Profile" && (
              <div style={{ maxWidth: 800, margin: "0 auto", display: "flex", flexDirection: "column", gap: 28 }}>
                <Card title="Display Name" icon="✏️">
                  <p style={{ fontSize: 14, color: "#64748b", marginBottom: 24, lineHeight: 1.6 }}>This name appears throughout the app.</p>
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    <input type="text" value={profile.name}
                      onChange={e => setProfile(p => ({ ...p, name: e.target.value }))}
                      placeholder="Enter your name"
                      style={{ flex: 1, background: "#f8fafc", border: "2px solid #e0f2fe", borderRadius: 14, padding: "15px 18px", fontSize: 18, color: "#0c4a6e", outline: "none", fontFamily: "inherit", fontWeight: 700, transition: "border-color 0.15s" }}
                      onFocus={e => e.target.style.borderColor = "#0ea5e9"}
                      onBlur={e => e.target.style.borderColor = "#e0f2fe"} />
                    <div style={{ padding: "15px 22px", background: "#f0f9ff", border: "1px solid #e0f2fe", borderRadius: 14, fontSize: 14, color: "#94a3b8", fontWeight: 600, whiteSpace: "nowrap" }}>
                      Preview: <span style={{ color: "#0c4a6e", fontWeight: 800 }}>{profile.name || "—"}</span>
                    </div>
                  </div>
                </Card>
      ...
      <ChatWidget mood={mood} profile={profile} />
    </>
  );
}
